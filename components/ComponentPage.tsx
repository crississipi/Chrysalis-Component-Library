...
const ComponentPage = ({ setViewComponentInfo, displayName, displayTags }: ComponentProps) => {
  ...
  return (
    <div className='h-[100vh] w-full flex flex-col gap-2 px-3 pt-10 z-10 items-start justify-start'>
      ...
      <div className='h-max w-full flex flex-col gap-3 p-2 pt-0'>
        <div className='w-full flex flex-col items-end'>
          <div className={`right-0 -bottom-3.5 text-xs font-medium flex rounded-t-xl ${!showOther ? 'bg-white' : 'bg-black'} overflow-hidden p-1.5 pb-0`}>
            <button type="button" className={`py-1.5 px-3 rounded-md ${!showOther ? 'bg-emerald-300' : 'text-white'} hover:bg-emerald-200 focus:bg-emerald-300 ease-in-out duration-200`} onClick={toggleShowOther}>Prev</button>
